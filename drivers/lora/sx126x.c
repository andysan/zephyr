...
LOG_MODULE_REGISTER(sx126x);


#if DT_HAS_COMPAT(semtech_sx1261)
#define DT_DRV_COMPAT semtech_sx1261
#define SX126X_DEVICE_ID SX1261
#elif DT_HAS_COMPAT(semtech_sx1262)
...
BUILD_ASSERT(DT_NUM_INST(semtech_sx1261) +
	     DT_NUM_INST(semtech_sx1262) <= 1,
...
#define GPIO_IRQ_LABEL		DT_INST_GPIO_LABEL(0, irq_gpios)
...
#if DT_INST_NODE_HAS_PROP(0, dio2_tx_enable)
#define DIO2_TX_ENABLE DT_INST_PROP(0, dio2_tx_enable)
#else
#define DIO2_TX_ENABLE 0
#endif
...
static int sx126x_spi_transceive(u8_t *req_tx, u8_t *req_rx, size_t req_len,
				 void *data_tx, void *data_rx, size_t data_len)
{
	int ret;
...
u8_t SX126xReadRegister(u16_t address)
{
	u8_t data;
...
void SX126xReadRegisters(u16_t address, u8_t *buffer, u16_t size)
{
	u8_t req[] = {
		RADIO_READ_REGISTER,
		(address >> 8) & 0xff,
...
void SX126xWriteRegister(u16_t address, u8_t value)
{
	SX126xWriteRegisters(address, &value, 1);
}

void SX126xWriteRegisters(u16_t address, u8_t *buffer, u16_t size)
{
	u8_t req[] = {
...
u8_t SX126xReadCommand(RadioCommands_t opcode,
			  u8_t *buffer, u16_t size)
{
	u8_t tx_req[] = {
		opcode,
		0x00,
...
void SX126xWriteCommand(RadioCommands_t opcode, u8_t *buffer, u16_t size)
{
	u8_t req[] = {
		opcode,
	};
...
void SX126xReadBuffer(u8_t offset, u8_t *buffer, u8_t size)
{
	u8_t req[] = {
		RADIO_READ_BUFFER,
		offset,
...
void SX126xWriteBuffer(u8_t offset, u8_t *buffer, u8_t size)
{
	u8_t req[] = {
		RADIO_WRITE_BUFFER,
		offset,
...
u32_t SX126xGetBoardTcxoWakeupTime(void)
{
	return TCXO_POWER_STARTUP_DELAY_MS;
}

u8_t SX126xGetDeviceId(void)
{
...
static void sx126x_irq_work_handler(struct k_work *work)
{
	...
	if (!dev_data.radio_dio_irq) {
		LOG_WRN("Radio IRQ before setting radio_dio_irq.");
		return;
	}
...
static void sx126x_irq_callback(struct device *dev,
				struct gpio_callback *cb, u32_t pins)
{
	if (pins & BIT(GPIO_IRQ_PIN)) {
...
static int sx126x_lora_init(struct device *dev)
{
	...
	dev_data.irq = device_get_binding(GPIO_IRQ_LABEL);
	if (!dev_data.irq) {
		LOG_ERR("Cannot get pointer to %s device", GPIO_IRQ_LABEL);
		return -EIO;
	}

	gpio_pin_configure(dev_data.irq, GPIO_IRQ_PIN,
			   GPIO_INPUT | GPIO_INT_DEBOUNCE | GPIO_IRQ_FLAGS);

	k_work_init(&dev_data.irq_work, sx126x_irq_work_handler);

	gpio_init_callback(&dev_data.irq_callback, sx126x_irq_callback,
			   BIT(GPIO_IRQ_PIN));
	if (gpio_add_callback(dev_data.irq, &dev_data.irq_callback) < 0) {
		LOG_ERR("Could not set GPIO callback for IRQ line.");
		return -EIO;
	}

	gpio_pin_interrupt_configure(dev_data.irq, GPIO_IRQ_PIN,
	...
	dev_data.radio_events.TxDone = sx12xx_ev_tx_done;
	dev_data.radio_events.RxDone = sx12xx_ev_rx_done;
	Radio.Init(&dev_data.radio_events);
