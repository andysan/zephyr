...
#define VALID_REQUIRED (			\
	VALID_CryptoNvmCtx |			\
	VALID_SecureElementNvmCtx |		\
	VALID_MacNvmCtx |			\
	VALID_RegionNvmCtx |			\
	VALID_CommandsNvmCtx |			\
	VALID_ConfirmQueueNvmCtx |		\
	0)
...
#define CTX(PFX)						\
	do {								\
		if (!strcmp(#PFX, key)) {				\
			...
			LOG_DBG("Loaded %s, valid mask 0x%x",		\
				#PFX, ctx->valid);			\
			return ret;					\
		}							\
	} while (0)
...
int lorawan_resume(void)
{
	...
	if (status != LORAMAC_STATUS_OK) {
		LOG_ERR("Failed to get LoRaMAC state: %s (%d)",
			lorawan_status2str(status), status);
	...
	if (status != LORAMAC_STATUS_OK) {
		LOG_ERR("Failed to set LoRaMAC state: %s (%d)",
			lorawan_status2str(status), status);
		return lorawan_status2errno(status);
	}
	...
	status = LoRaMacStart();
	if (status != LORAMAC_STATUS_OK) {
		LOG_ERR("Failed to start the stack: %s (%d)",
			lorawan_status2str(status), status);
		return lorawan_status2errno(status);
	}
...
int lorawan_suspend(void)
{
	...
	status = LoRaMacStop();
	if (status != LORAMAC_STATUS_OK) {
		LOG_ERR("Failed to stop the LoRaMAC stack: %s (%d)",
			lorawan_status2str(status), status);
		return lorawan_status2errno(status);
	}
